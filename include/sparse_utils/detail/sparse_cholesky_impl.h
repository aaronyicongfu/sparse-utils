...
void SparseCholesky<T>::construct_cholesky(int _size, const int *Acolp,
  ...
  if (order == CholOrderingType::ND) {
    ...
    delete[] copy_Acolp;
    delete[] copy_Arows;
  } else {  // order == CholOrderingType::NATURAL
...
*/
template <typename T>
void SparseCholesky<T>::getInfo(int *_size, int *_num_snodes, int *_nnzL) {
  if (_size) {
    *_size = size;
  }
  if (_num_snodes) {
...
*/
template <typename T>
void SparseCholesky<T>::buildForest(const int Acolp[], const int Arows[],
                                    int parent[], int Lnz[]) {
...
*/
template <typename T>
int SparseCholesky<T>::initSupernodes(const int parent[], const int Lnz[],
                                      int vtn[]) {
  int snode = 0;
  for (int i = 0; i < size;) {
    vtn[i] = snode;
...
*/
template <typename T>
void SparseCholesky<T>::buildNonzeroPattern(const int Acolp[],
                                            const int Arows[],
...
*/
template <typename T>
void SparseCholesky<T>::updateDiag(const int lsize, const int nlrows,
                                   const int lfirst_var, const int *lrows, T *L,
                                   const int diag_size, T *diag, T *work) {
  // Compute L * L^{T}
  int n = nlrows;
  int k = lsize;
...
*/
template <typename T>
void SparseCholesky<T>::updateWorkColumn(int lwidth, int n21rows, T *L21,
                                         int n31rows, T *L31, T *temp) {
...
*/
template <typename T>
void SparseCholesky<T>::updateColumn(const int lwidth, const int nlcols,
                                     const int lfirst_var, const int *lrows,
                                     int nrows, const int *arows, const T *A,
                                     const int *brows, T *B) {
...
*/
template <typename T>
void SparseCholesky<T>::solveDiagTranspose(int diag_size, T *L, int nrhs,
                                           T *x) {
  int incr = 1;
  for (int k = 0; k < nrhs; k++) {
...
*/
template <typename T>
void SparseCholesky<T>::solve(T *x) {
  T *xt = x;
...
#endif  // SPARSE_UTILS_SPARSE_CHOLESKY_IMPL_H
